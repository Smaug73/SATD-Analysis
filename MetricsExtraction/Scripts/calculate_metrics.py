...
def traverse_commits(directory, projects):
    for project in projects:
        ...
        print('\nBranches:')
        # Get the names of all the branches different from the main one
        ...
        # Set for ignoring commits already analysed
        ...
        # Calculate metrics for each commit in the main branch
        measure(rm, commits_analysed, tot_commits, rows)


        # Calculate metrics for each commit in other branches
        for br in branches:
            print('\nBranch: ' + br)
...
# From each method of each added or modified file in each commit, retrieve 3 metrics: 
# 1. Number of parameters 
# 2. NLOC 
# 3. Complexity
# NB: PyDriller provides fan-in and fan-out for each method too, 
# but it probably computes them in a wrong way since their values are always 0.
...
if __name__ == "__main__":
    ...
    # Names of projects to be analysed
